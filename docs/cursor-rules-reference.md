# AI Engineering Governance Rules — Full Reference

This document contains the complete rules with code examples. It is **not** injected into every prompt. Use it when you need detailed patterns or examples. The mandatory short rules are in `.cursor/rules.md`.
...
1. **Folder placement is unclear** — You're not sure which folder a file belongs in; the file doesn't fit existing folder structure.
2. **A rule violation is required** — The user's request would violate architectural rules; explain the conflict and ask for guidance.
3. **New architectural patterns are requested** — User wants to add new folders, change structure, or modify the layering approach.
4. **Constants or config usage is ambiguous** — Not sure if a value should be a constant or how to structure a new constant file.
...
These rules exist to prevent technical debt and ensure maintainable, scalable codebases.
